...
RegressionTree::getPartitions_Cached_Buggy(int vId, double& splitValue,double marginalEntropy, vector<int>& dataSet, double& infoGain)
{
	...
	for(int dIter=0;dIter<dataSet.size();dIter++)
	{
		...
		//SR:: Removing this check but be pre-warned
...
RegressionTree::estimateMarginal()
{
	...
	//getSubEntropy_NB(dataSubset,0,dataSubset.size()-1,mean,variance,marginalEntropy);
	return 0;
}
...
int
RegressionTree::getSubEntropy_NB(vector<int>& sortedInd, int start, int end,double& mean,double& variance,double& marginalEntropy)
{
	NBWrapper nbw;
	...
	for(int i=start;i<=end;i++)
	{
		...
		double classVarVal=(*evMap)[classVarID]->getEvidVal();
		countValues.push_back((int)classVarVal);
	}
	mean=0;	
	//The variance is actually set to the prob of an interaction
	nbw.estimateParams(mean,variance,countValues);
	countValues.clear();
	return 0;
}
