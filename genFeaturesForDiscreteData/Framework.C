...
Framework::splitRegionsGenPairs(int foldCnt,const char* outputDir)
{
	...
	//loop over the regions to precompute the mean for windows features, store it and perform k-means clustering
	cout << "Precomputing windows..."<<endl;
	int pairID = 0;
		...
		for(map<string,map<string,double>*>::iterator nIter=neighbors->begin();nIter!=neighbors->end();nIter++)
			{
				...
				int neighborsize=(*nIter->second)[pairFeatureOrder[0]]/BinSize;
				//this function will store the values into windowMeans
				//pass it a key for the pair -- is pIter->first the pairname> 
				stringstream pk;
				pk << pIter->first << string("-") << nIter->second;
				string pairKey = pk.str();

				pairIDs[pairKey]=pairID;
				showFeaturesPairNeighbors(pairID, enfeatures,neighborsize);
				pairID=pairID+1;
			}
	}

	//We can switch the IDs for kmeansData to be <feature<exampleID,value>>
	//We want <<featureID<dimension, value>> 
	//Loop over the keys of feature, create a map of <featureID <dimension,value>> and compute

	cout << "Done precomputing windows.." << endl;

	for(map<int,map<int,double>*>::iterator kIter=kmeansData.begin();kIter!=kmeansData.end();kIter++)
	{
		int featID = kIter->first;
		string featName = featureOrder[featID];
		Dataset* d=datasetsContinuous[featName];
		char type=d->returnType();

		if (type == 'W')
		{
		map<int, double> dataSet = (*kIter->second);
		map<int,map<int,double>*> ds;
		for(map<int,double>::iterator eIter=dataSet.begin();eIter!=dataSet.end();eIter++)
		{
			int ID = eIter->first;
			double val = eIter->second;
			map<int,double>* x = new map<int,double>;
			(*x)[0]=val;
			ds[ID]=x;
		}		
		...
		kmeans->setClusterCnt(20);
		kmeans->setDistanceType(Kmeans::EUCLID);
		kmeans->setMaxIter(100);
		kmeans->cluster(&ds);
		CLUSTERS& clusters=kmeans->getClusters();
		allClusters[featID]=clusters;
		cout << "Done computing kmeans.." << endl;
		}
	}
	cout << "Re-structuring cluster data..." << endl;

	//map<int,vector<int>> <- pairID, windows in feature order
	//loop over the features - loop over the 20 vectors indicating the cluster
	// use the value in the vector as the featID key and add the cluster ID to the new vector
	// map windowFeatures[pairID] = 0.7 4 5 ..

	//loop over the featureOrder
	for(int i=0;i<featureOrder.size();i++)
	{
		string featName = featureOrder[i];
		Dataset* d=datasetsContinuous[featName];
		char type=d->returnType();
		...
		//get the cluster data for the feature - cluster ID and vector of members
		map<int,vector<int>*> featureClusters = allClusters[i];

		//iterate over the clusters and get the ID and members  for the feature
		for(map<int,vector<int>*>::iterator cIter=featureClusters.begin();cIter!=featureClusters.end();cIter++)
		{
			int cluster=cIter->first;
			vector<int> members=(*cIter->second);
			int n=0;
			
			//loop over the members 
			for(vector<int>::iterator it = members.begin(); it != members.end(); ++it, ++n)
			{
				//if window features does not have a value for this pair
				if(windowFeatures.find(*it)==windowFeatures.end())
				{
					map<int,double>* featureMeans = new map<int, double>;
					(*featureMeans)[i]=cluster;
					windowFeatures[*it]=featureMeans;
				}else
				{
					map<int,double>* featureMeans=windowFeatures[*it];
					(*featureMeans)[i]=cluster;
					windowFeatures[*it]=featureMeans;
				}
			} 
		...
		if (type=='M')
		{
			//kmeansData[i] is the <featureID, value> for feature i
			map<int,double> featMap = (*kmeansData[i]);
			for(map<int,double>::iterator cIter=featMap.begin();cIter!=featMap.end();cIter++)
			{
				int featID=cIter->first;
				double value=cIter->second;
				if(windowFeatures.find(featID)==windowFeatures.end())
				{
					map<int,double>* featureMeans = new map<int, double>;
					(*featureMeans)[i]=value;
					windowFeatures[featID]=featureMeans;
				}else
				{
					map<int,double>* featureMeans=windowFeatures[featID];
					(*featureMeans)[i]=value;
					windowFeatures[featID]=featureMeans;
				}
			}
	...
	cout << "Done re-structuring cluster data..." << endl;
	...
	//	char test3FName[1024];
		...
		for(map<string,map<string,map<string,double>*>*>::iterator pIter=pairSet_Filtered.begin();pIter!=pairSet_Filtered.end();pIter++)
		{
			...
			for(map<string,map<string,double>*>::iterator nIter=neighbors->begin();nIter!=neighbors->end();nIter++)
			{
				...
				pk << pIter->first << string("-") << nIter->second;
				string pairKey = pk.str();
				/*if( (eID>=testBegin && eID<testEnd) && (pID>=testBegin && pID<testEnd))
				{
					teFile <<pIter->first<<"-"<<nIter->first;
					showFeaturesPair(teFile,efeatures,pfeatures,nIter->second);
				}
				else if((eID<testBegin|| eID>testEnd) && (pID<testBegin||pID>testEnd) )
				{
				...
				//enhancer and promoter IDs 
				if( (testID.find(eID)!=testID.end()) && (testID.find(pID)!=testID.end()))
				{
					teFile <<pIter->first<<"-"<<nIter->first;
					showFeaturesPair(teFile,efeatures,pfeatures,enfeatures,nIter->second, pairKey);
				}
				else if( (trainID.find(eID)!=trainID.end()) && (trainID.find(pID)!=trainID.end()))
				{
					trFile <<pIter->first<<"-"<<nIter->first;
					showFeaturesPair(trFile,efeatures,pfeatures,enfeatures,nIter->second, pairKey);
				}
				else if( (trainID.find(eID)!=trainID.end()) )
				{
					test1File <<pIter->first<<"-"<<nIter->first;
					showFeaturesPair(test1File,efeatures,pfeatures,enfeatures,nIter->second, pairKey);
				}
				else if( (trainID.find(pID)!=trainID.end()) )
				{
					test2File <<pIter->first<<"-"<<nIter->first;
					showFeaturesPair(test2File,efeatures,pfeatures,enfeatures,nIter->second, pairKey);

				}/*
				else{	
					test3File <<pIter->first<<"-"<<nIter->first;
					showFeaturesPair(test3File,efeatures,pfeatures,nIter->second);
				}*/
...
Framework::splitRegionsGenPairs_NoPrerandomize(int foldCnt,const char* outputDir)
{
	...
	cout << "Done re-structuring cluster data..." << endl;
		/*	
		for(map<int,int> ::iterator nIter=testID.begin();nIter!=testID.end();nIter++)
		{
			cout << nIter->first << " -> " << testID[nIter->first]<<endl;
...
Framework::readFeatures(const char* aFName)
{
	...
	while(inFile.good())
	{
		...
		{
			Dataset* d=new Dataset;
			if(datasetsContinuous.find(featName)==datasetsContinuous.end())
			{
				//featureOrder.push_back(featName);
				datasetsContinuous[featName]=d;
				d->readDataSet(fileName,type);
			}
			else
			{
				d=datasetsContinuous[featName];
				d->readDataSet(fileName, type);
...
{
	char aFName_E[1024];
	sprintf(aFName_E,"%s/_region.txt",aFName);
...
Framework::showHeader(ofstream& oFile)
{
	...
	for(int f=0;f<featureOrder.size();f++)
	{
		if(featureOrder[f].compare("CTCF_PLUS") == 0|| featureOrder[f].compare("CTCF_MINUS") == 0){
			continue;
		}
		else
		{
	...
	if(featype.compare("CTCFdirect")==0 || featype.compare("CTCFprod")==0){
		oFile << "\t" << "Eplus_Pplus" << "\t" << "Eplus_Pminus" << "\t" << "Eminus_Pplus" << "\t" << "Eminus_Pminus" ;		
	}
	else if (featype.compare("Outerprod")==0){
		for(int f=0;f<featureOrder.size();f++)
		{	
...
//this is not the one with window feature - ignore
int
Framework::showFeaturesPair(ofstream& oFile, map<string,double>* efeatures, map<string,double>* pfeatures,map<string,double>* pairFeatures)
{
	...
	for(int f=0;f<featureOrder.size();f++)
	{
		...
		{
			featVal_p=(*pfeatures)[featureOrder[f]]+0;
		}
		oFile <<"\t" << featVal_p;
	}
	for(int f=0;f<pairFeatureOrder.size();f++)
...
//this is the one we edit 
// added by Shilu to generate CTCF directionality:
int
Framework::showFeaturesPair(ofstream& oFile, map<string,double>* efeatures, map<string,double>* pfeatures,map<string,double>* enfeatures,map<string,double>* pairFeatures, string pairKey)
{
	double Eplus=0;
	double Eminus=0;
	...
	//map<string,double>* enfeatures=regionNeighborFeatures[pIter->first];
        ...
        {
		double featVal_e=0;
		if(featureOrder[f].compare("CTCF_PLUS") == 0 )
		{
			Eplus=(*efeatures)[featureOrder[f]];
			
		}
		else if(featureOrder[f].compare("CTCF_MINUS") == 0)
		{
			Eminus=(*efeatures)[featureOrder[f]];
		}
		else
		{
		...
		if(featureOrder[f].compare("CTCF_PLUS") == 0)
                {
                        Pplus=(*pfeatures)[featureOrder[f]];
                        
                }
                else if(featureOrder[f].compare("CTCF_MINUS") == 0)
                {
                        Pminus=(*pfeatures)[featureOrder[f]];
                }
                else
                {
                	...
                	if(pfeatures->find(featureOrder[f])!=pfeatures->end())
                	{
                        featVal_p=(*pfeatures)[featureOrder[f]];
                	}
                	oFile <<"\t" << featVal_p;
		}
        }
	...
	if(featype.compare("CTCFdirect")==0){
		// calculate ratio for ++, +-, -+ or --:
		if (Eplus+Pplus==0){
			featVal=0;
		}
		else{
			featVal=Eplus/(Eplus+Pplus);  //++
		}
		oFile <<"\t"  <<featVal;
		if(Eplus+Pminus==0){
			featVal=0;
		...
		if(Eminus+Pminus==0){
			...
			featVal=Eminus/(Eminus+Pminus);  //--
		}
		oFile <<"\t"  <<featVal;
	
	}
	else if(featype.compare("CTCFprod")==0){
		featVal=Eplus*Pplus; 
		oFile <<"\t"  <<featVal;
		featVal=Eplus*Pminus;
		oFile <<"\t"  <<featVal;
		featVal=Eminus*Pplus;
		oFile <<"\t"  <<featVal;
		featVal=Eminus*Pminus; 
		oFile <<"\t"  <<featVal;
	}
	else if(featype.compare("Outerprod")==0)
	{	
	...
	}else if(featype.compare("Window")==0)
	{
		...
		//vector<double> meanVec = windowMeans[pairKey];
		int ID=pairIDs[pairKey];
		showFeaturesPairNeighbors(oFile, ID);
...
//computes all the window features for one pair
int
Framework::showFeaturesPairNeighbors(int pairname, map<string,double>* enfeatures,int neighborsize)
{
	//For feature X
	for(int i=0;i<featureOrder.size();i++)
	{
		...
		//This loop gets the values of feature X for each neighbor
		...
		if(neighborsize>0){
			...
			//store featVal_e here once the fold is done where we can access it by the featName windowMeans[featName] - global variable for framewor		
		}
		else{
			featVal_e=0.01;
		}
		...
		if(kmeansData.find(i)==kmeansData.end())
		{
			map<int,double>* featureMeans = new map<int, double>;
			(*featureMeans)[pairname]=featVal_e;
			kmeansData[i]=featureMeans;
		}else
		{
			map<int,double>* featureMeans=kmeansData[i];
			(*featureMeans)[pairname]=featVal_e;
			kmeansData[i]=featureMeans;
		}
...
int
Framework::showFeaturesPairNeighbors(ofstream& oFile, int pairKey)
{
	for(int i=0;i<featureOrder.size();i++)
	{
		map<int,double> featMap = (*windowFeatures[pairKey]);
		oFile <<"\t" << featMap[i];
	}
        return 0;
}
...
int
Framework::populateFeaturesForRegionContinuous(map<string,double>* populateMe, string& key)
{
	...
	for(map<string,Dataset*>::iterator dIter=datasetsContinuous.begin();dIter!=datasetsContinuous.end();dIter++)
	{
...
Framework::filterRegions()
{
	...
	for(map<string,Region*>::iterator rIter=regionSet.begin();rIter!=regionSet.end();rIter++)
	{
		...
		map<string,double>* efeatures=NULL;
		map<string,double>* enfeatures=NULL;
		if(regionFeatures.find(e)==regionFeatures.end())
		{
			efeatures=new map<string,double>;
			enfeatures=new map<string,double>;
			populateFeaturesForRegion(efeatures,e);
			populateFeaturesForRegionContinuous(enfeatures,e);
			if(efeatures->size()>0)
			{
			...
			if(enfeatures->size()>0)
			{
				regionFeaturesContinuous[e]=enfeatures;
			}
			else
			{
				delete enfeatures;
...
{
	//int bin=5000;
	for(map<string,map<string,double>*>::iterator rIter=regionFeaturesContinuous.begin();rIter!=regionFeaturesContinuous.end();rIter++)
	{
		...
		/* skip previous neighbors
		while(k<=regionDistNeighborSize)
		{
			int neighCoord=r->begin-(k*5000);
			char neighborName[1024];
			sprintf(neighborName,"%s_%d_%d",r->chromosome.c_str(),neighCoord,neighCoord+5000);
		...
		while(k<=regionDistNeighborSize)
		{
			...
			string key(neighborName);
			if(regionFeaturesContinuous.find(key)==regionFeaturesContinuous.end())
			{
				k++;
				continue;		
			}
			map<string,double>* neighborFeatures=regionFeaturesContinuous[key];
			for(map<string,double>::iterator sIter=neighborFeatures->begin();sIter!=neighborFeatures->end();sIter++)
			{
				char fKey[1024];
				sprintf(fKey,"%s+%d",sIter->first.c_str(),k);
...
main(int argc, const char** argv)
{
	...
	if(argc!=10)
	{
		cout <<"Usage: getFeatures sparseMatrix dist foldcv splittype[regionwise|pairwise] featurefile correlation[yes|no] outputdir prerandomize_pairs[yes|no] featuretype[Directionality|..]" << endl;
		return 0;
	}
	Framework fw;
